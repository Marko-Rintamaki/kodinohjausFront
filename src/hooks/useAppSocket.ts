...
import { initializeSocket, disconnectSocket, sendDataQuery, sendControlCommand, updateGlobalUpdateStatus } from '../helpers/socketHelper';
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      initializeSocket(token, {
        onConnect: () => {
          console.log('🔍 [useAppSocket] KRIITTINEN: Socket yhdistetty!');
          if (appSocketLogging) {
        ...
        onStatusUpdate: (status) => {
          console.log('🔍 [useAppSocket] KRIITTINEN: statusUpdate vastaanotettu:', status);
          if (appSocketLogging) {
            console.log('[HOOK] statusUpdate received:', status);
          }
