...
export interface LEDStripPoint { x:number; y:number; }
...
export const LEDStrip: React.FC<LEDStripProps> = ({
  ...
  const toggle = useCallback(()=>{ const next=!active; if(!controlled) setInternalOn(next); onChange?.(next); onToggle?.(); },[active,controlled,onChange,onToggle]);
  ...
  const absPts = useMemo(()=> points.map(p=>({ x:p.x*baseWidth, y:p.y*baseHeight })), [points, baseWidth, baseHeight]);
  ...
  <path className="ledstrip-path" d={pathD} style={{ stroke: showPath ? pathColor : 'transparent' }} />
  {showSpots && spots.map((s,i)=>(<circle key={i} className="ledstrip-spot" cx={s.x} cy={s.y} r={spotRadius} />))}
