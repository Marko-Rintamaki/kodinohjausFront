...
var socketConnectionLogging = false;
var socketTokenLogging = false;
var socketOutgoingLogging = false;
var socketIncomingLogging = false;
var updateStatusLogging = true;
var dataQueryLogging = false;
...
// Sisäinen funktio updateStatus:n päivittämiseen
const updateGlobalStatus = (newStatus: any): void => {
  ...
  updateStatusCallbacks.forEach(callback => {
    try {
      callback(newStatus);
    } catch (error) {
...
const setupSocketListeners = (callbacks: SocketCallbacks) => {
  ...
  const STATUS_LOGGING_ENABLED = false;
